...
export default function Page() {
  ...
  async function load(){
    ...
    const {data:r}=await supabase.from('daily_care_records').select('*').order('created_at',{ascending:false}).limit(200);
    setRows(r||[]);
  }
  async function save(){
  ...
  async function complete(id:string){
    await supabase.from('daily_care_records').update({status:'Tamamlandı', completed_at:new Date().toISOString()}).eq('id',id);
    await load();
  }
  return <Shell>
    <div className="hero"><h2>Günlük Bakım Takibi</h2><p>Banyo, bez değişimi, mobilizasyon, pozisyon, uyku ve genel bakım kaydı.</p></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Yeni Kayıt</h2><p>Hasta seçerek kayıt oluştur.</p></div></div>
      <div className="grid grid4"><div><label>Hasta</label><select value={form.patient_id} onChange={e=>setForm({...form,patient_id:e.target.value})}>{patients.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></div><div><label>Bakım Zamanı</label><input type="datetime-local" value={form.care_at} onChange={e=>setForm({...form,care_at:e.target.value})} /></div><div><label>Bakım Türü</label><select value={form.care_type} onChange={e=>setForm({...form,care_type:e.target.value})}><option>Banyo</option><option>Bez Değişimi</option><option>Pozisyon</option><option>Mobilizasyon</option><option>Ağız Bakımı</option><option>Tırnak Bakımı</option><option>Genel Bakım</option><option>Diğer</option></select></div><div><label>Durum</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Tamamlandı</option><option>Bekliyor</option><option>Reddetti</option><option>Sorunlu</option></select></div><div><label>Yapan Kişi</label><input value={form.performed_by} onChange={e=>setForm({...form,performed_by:e.target.value})} /></div><div><label>Not</label><textarea value={form.note} onChange={e=>setForm({...form,note:e.target.value})} /></div></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Kayıtlar</h2><p>Son 200 kayıt</p></div></div>
      <div className="tableWrap"><table><thead><tr><th>Zaman</th><th>Hasta</th><th>Bakım Zamanı</th><th>Bakım Türü</th><th>Durum</th><th>Yapan Kişi</th><th>Not</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString('tr-TR')} </td><td><b>{r.patient_name}</b></td><td>{r.care_at||'-'}</td><td>{r.care_type||'-'}</td><td>{r.status||'-'}</td><td>{r.performed_by||'-'}</td><td>{r.note||'-'}</td><td><span className={`pill ${r.status==='Tamamlandı'||r.status==='Aktif'||r.status==='Normal'?'ok':'warn'}`}>{r.status||'-'}</span></td></td><td>{r.status!=='Tamamlandı'?<button className="green" onClick={()=>complete(r.id)}>Tamamla</button>:'-'}</td></tr>)}{rows.length===0&&<tr><td colSpan={9}>Kayıt yok.</td></tr>}</tbody></table></div>
    </div>
  </Shell>
}
