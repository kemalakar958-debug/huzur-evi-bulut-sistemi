...
export default function Page() {
  ...
  const [form,setForm]=useState<any>({patient_id:'',record_at:'',meal_type:'',intake_status:'',fluid_ml:'',diet_note:'',status:''});
  ...
  async function load(){
    ...
    const {data:r}=await supabase.from('nutrition_records').select('*').order('created_at',{ascending:false}).limit(200);
    setRows(r||[]);
  }
  async function save(){
  ...
  async function complete(id:string){
    await supabase.from('nutrition_records').update({status:'Tamamlandı', completed_at:new Date().toISOString()}).eq('id',id);
    await load();
  }
  return <Shell>
    <div className="hero"><h2>Beslenme / Sıvı Takibi</h2><p>Öğün, iştah, sıvı alımı ve özel diyet takibi.</p></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Yeni Kayıt</h2><p>Hasta seçerek kayıt oluştur.</p></div></div>
      <div className="grid grid4"><div><label>Hasta</label><select value={form.patient_id} onChange={e=>setForm({...form,patient_id:e.target.value})}>{patients.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></div><div><label>Zaman</label><input type="datetime-local" value={form.record_at} onChange={e=>setForm({...form,record_at:e.target.value})} /></div><div><label>Öğün</label><select value={form.meal_type} onChange={e=>setForm({...form,meal_type:e.target.value})}><option>Kahvaltı</option><option>Öğle</option><option>Akşam</option><option>Ara Öğün</option><option>Sıvı</option></select></div><div><label>Alım Durumu</label><select value={form.intake_status} onChange={e=>setForm({...form,intake_status:e.target.value})}><option>Tam Aldı</option><option>Yarım Aldı</option><option>Az Aldı</option><option>Reddetti</option><option>Kusma</option></select></div><div><label>Sıvı ml</label><input type="number" value={form.fluid_ml} onChange={e=>setForm({...form,fluid_ml:e.target.value})} /></div><div><label>Diyet / Not</label><textarea value={form.diet_note} onChange={e=>setForm({...form,diet_note:e.target.value})} /></div><div><label>Durum</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Normal</option><option>Takip</option><option>Sorunlu</option></select></div></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Kayıtlar</h2><p>Son 200 kayıt</p></div></div>
      <div className="tableWrap"><table><thead><tr><th>Zaman</th><th>Hasta</th><th>Zaman</th><th>Öğün</th><th>Alım Durumu</th><th>Sıvı ml</th><th>Diyet / Not</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString('tr-TR')} </td><td><b>{r.patient_name}</b></td><td>{r.record_at||'-'}</td><td>{r.meal_type||'-'}</td><td>{r.intake_status||'-'}</td><td>{r.fluid_ml||'-'}</td><td>{r.diet_note||'-'}</td><td><span className={`pill ${r.status==='Tamamlandı'||r.status==='Aktif'||r.status==='Normal'?'ok':'warn'}`}>{r.status||'-'}</span></td></td><td>{r.status!=='Tamamlandı'?<button className="green" onClick={()=>complete(r.id)}>Tamamla</button>:'-'}</td></tr>)}{rows.length===0&&<tr><td colSpan={9}>Kayıt yok.</td></tr>}</tbody></table></div>
    </div>
  </Shell>
}
