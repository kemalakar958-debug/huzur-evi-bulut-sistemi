...
export default function Page() {
  ...
  const [form,setForm]=useState<any>({patient_id:'',dose_time:'',medication_name:'',scheduled_date:'',status:'',note:''});
  ...
  async function load(){
    ...
    const {data:r}=await supabase.from('mar_records').select('*').order('created_at',{ascending:false}).limit(200);
    setRows(r||[]);
  }
  async function save(){
    ...
    const {error}=await supabase.from('mar_records').insert(payload);
    if(error)return alert(error.message);
    setForm((o:any)=>{ const n:any={...o}; for(const k of Object.keys(n)) if(k!=='patient_id') n[k]=''; return n; });
    await load();
  }
  async function complete(id:string){
    await supabase.from('mar_records').update({status:'Tamamlandı', completed_at:new Date().toISOString()}).eq('id',id);
    await load();
  }
  return <Shell>
    <div className="hero"><h2>MAR İlaç Uygulama</h2><p>Sabah, öğle, akşam, gece ilaç uygulama kaydı.</p></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Yeni Kayıt</h2><p>Hasta seçerek kayıt oluştur.</p></div></div>
      <div className="grid grid4"><div><label>Hasta</label><select value={form.patient_id} onChange={e=>setForm({...form,patient_id:e.target.value})}>{patients.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></div><div><label>Doz Zamanı</label><select value={form.dose_time} onChange={e=>setForm({...form,dose_time:e.target.value})}><option>Sabah</option><option>Öğle</option><option>Akşam</option><option>Gece</option></select></div><div><label>İlaç</label><input value={form.medication_name} onChange={e=>setForm({...form,medication_name:e.target.value})} /></div><div><label>Tarih</label><input type="date" value={form.scheduled_date} onChange={e=>setForm({...form,scheduled_date:e.target.value})} /></div><div><label>Durum</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Verildi</option><option>Verilmedi</option><option>Reddetti</option><option>Bekliyor</option></select></div><div><label>Not</label><textarea value={form.note} onChange={e=>setForm({...form,note:e.target.value})} /></div></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Kayıtlar</h2><p>Son 200 kayıt</p></div></div>
      <div className="tableWrap"><table><thead><tr><th>Zaman</th><th>Hasta</th><th>Doz Zamanı</th><th>İlaç</th><th>Tarih</th><th>Durum</th><th>Not</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString('tr-TR')} </td><td><b>{r.patient_name}</b></td><td>{r.dose_time||'-'}</td><td>{r.medication_name||'-'}</td><td>{r.scheduled_date||'-'}</td><td>{r.status||'-'}</td><td>{r.note||'-'}</td><td><span className={`pill ${r.status==='Tamamlandı'||r.status==='Aktif'||r.status==='Normal'?'ok':'warn'}`}>{r.status||'-'}</span></td></td><td>{r.status!=='Tamamlandı'?<button className="green" onClick={()=>complete(r.id)}>Tamamla</button>:'-'}</td></tr>)}{rows.length===0&&<tr><td colSpan={9}>Kayıt yok.</td></tr>}</tbody></table></div>
    </div>
  </Shell>
}
