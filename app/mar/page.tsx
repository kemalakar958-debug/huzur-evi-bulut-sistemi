...
export default function Page() {
  ...
  return (
    <Shell>
      <div className="hero">
        <h2>MAR İlaç Uygulama</h2>
        <p>Sabah, öğle, akşam, gece ilaç uygulama kaydı.</p>
      </div>
      ...
      <div className="panel">
        ...
        <div className="grid grid4">
          ...
          <div>
            <label>Doz Zamanı</label>
            <input type="input" value={form.dose_time} onChange={(e) => setForm({ ...form, dose_time: e.target.value })} />
          </div>
          <div>
            <label>İlaç</label>
            <input type="input" value={form.medication_name} onChange={(e) => setForm({ ...form, medication_name: e.target.value })} />
          </div>
          <div>
            <label>Tarih</label>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{formatValue(row.scheduled_date)}</td>
                  <td>{formatValue(row.dose_time)}</td>
                  <td>{formatValue(row.medication_name)}</td>
