...
export default function Page() {
  ...
  async function load(){
    ...
    const {data:r}=await supabase.from('incident_records').select('*').order('created_at',{ascending:false}).limit(200);
    setRows(r||[]);
  }
  async function save(){
  ...
  async function complete(id:string){
    await supabase.from('incident_records').update({status:'Tamamlandı', completed_at:new Date().toISOString()}).eq('id',id);
    await load();
  }
  return <Shell>
    <div className="hero"><h2>Olay / Düşme Kaydı</h2><p>Düşme, yaralanma, kriz ve kurum içi olay kaydı.</p></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Yeni Kayıt</h2><p>Hasta seçerek kayıt oluştur.</p></div></div>
      <div className="grid grid4"><div><label>Hasta</label><select value={form.patient_id} onChange={e=>setForm({...form,patient_id:e.target.value})}>{patients.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></div><div><label>Olay Zamanı</label><input type="datetime-local" value={form.incident_at} onChange={e=>setForm({...form,incident_at:e.target.value})} /></div><div><label>Olay Türü</label><select value={form.incident_type} onChange={e=>setForm({...form,incident_type:e.target.value})}><option>Düşme</option><option>Yaralanma</option><option>Kriz / Nöbet</option><option>Davranışsal Olay</option><option>Kaybolma Riski</option><option>Diğer</option></select></div><div><label>Yer</label><input value={form.location} onChange={e=>setForm({...form,location:e.target.value})} /></div><div><label>Şiddet</label><select value={form.severity} onChange={e=>setForm({...form,severity:e.target.value})}><option>Hafif</option><option>Orta</option><option>Ciddi</option><option>Kritik</option></select></div><div><label>Açıklama</label><textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} /></div><div><label>Durum</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Aktif</option><option>Tamamlandı</option><option>Takip</option></select></div></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Kayıtlar</h2><p>Son 200 kayıt</p></div></div>
      <div className="tableWrap"><table><thead><tr><th>Zaman</th><th>Hasta</th><th>Olay Zamanı</th><th>Olay Türü</th><th>Yer</th><th>Şiddet</th><th>Açıklama</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString('tr-TR')} </td><td><b>{r.patient_name}</b></td><td>{r.incident_at||'-'}</td><td>{r.incident_type||'-'}</td><td>{r.location||'-'}</td><td>{r.severity||'-'}</td><td>{r.description||'-'}</td><td><span className={`pill ${r.status==='Tamamlandı'||r.status==='Aktif'||r.status==='Normal'?'ok':'warn'}`}>{r.status||'-'}</span></td></td><td>{r.status!=='Tamamlandı'?<button className="green" onClick={()=>complete(r.id)}>Tamamla</button>:'-'}</td></tr>)}{rows.length===0&&<tr><td colSpan={9}>Kayıt yok.</td></tr>}</tbody></table></div>
    </div>
  </Shell>
}
