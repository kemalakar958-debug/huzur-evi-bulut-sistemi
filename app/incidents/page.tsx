...
export default function IncidentsPage() {
  ...
  async function load() {
    ...
    incidentQuery = applyActiveFacilityFilter(incidentQuery, ctx);

    const { data: incidentRows } = await incidentQuery;
  ...
  return (
    <Shell>
      <div className="hero">
        <h2>Olay / Düşme</h2>
        <p>Aktif kurum filtresine bağlı olay kayıtları.</p>
      </div>
      ...
      <div className="panel">
        <div className="grid grid4">
          ...
          <div>
            ...
            <select value={form.incident_type} onChange={(e) => setForm({ ...form, incident_type: e.target.value })}>
              <option>Düşme</option>
              <option>Yaralanma</option>
              <option>Kriz</option>
              <option>Davranışsal Olay</option>
              <option>Diğer</option>
            </select>
          </div>
        ...
        <div className="actions">
          <button className="primary" onClick={save}>Olay Kaydet</button>
        </div>
      </div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.incident_at ? new Date(r.incident_at).toLocaleString('tr-TR') : '-'}</td>
