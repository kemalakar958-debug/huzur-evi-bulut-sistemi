...
export default function Patient360Page({ params }: { params: { id: string } }) {
  ...
  return (
    <>
        ...
        <div className="p360Grid2">
          <div className="p360Panel">
            ...
            <div className="p360FormGrid">
              ...
              <div>
                <label>İlaç Başlangıç Tarihi</label>
                <input type="date" value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} />
              </div>

              <div>
                <label>Başlangıç Stok</label>
                <input type="number" value={form.initial_stock} onChange={(e) => setForm({ ...form, initial_stock: e.target.value })} placeholder="Örn: 112" />
              </div>

              <div>
                <label>Günlük Doz</label>
                <input type="number" step="0.01" value={form.daily_dose} onChange={(e) => setForm({ ...form, daily_dose: e.target.value })} placeholder="Örn: 2" />
              </div>
              ...
              <div>
                <label>Rapor Başlangıç</label>
                <input type="date" value={form.report_start_date} onChange={(e) => setForm({ ...form, report_start_date: e.target.value })} />
              </div>

              <div>
                <label>Rapor Bitiş</label>
                <input type="date" value={form.report_end_date} onChange={(e) => setForm({ ...form, report_end_date: e.target.value })} />
              </div>
                ...
                {meds.map((m) => {
                  ...
                  return (
                    <tr key={m.id}>
                      ...
                      <td>
                        <span className={`p360Badge ${statusClass(c.remainingDays)}`}>
                          {c.remaining}
                        </span>
                        <br />
                        <small>{c.remainingDays !== null ? `${c.remainingDays} gün` : '-'}</small>
                      </td>
                      <td>{c.estimatedEndDate ? fmtDate(c.estimatedEndDate) : '-'}</td>
                      <td>
                        <span className={`p360Badge ${statusClass(c.expiryDays)}`}>
                          {m.expiry_date ? fmtDate(m.expiry_date) : '-'}
                        </span>
                        <br />
                        <small>{c.expiryDays !== null ? `${c.expiryDays} gün` : '-'}</small>
                      </td>
                      <td>
                        <span className={`p360Badge ${statusClass(c.reportDays)}`}>
                          {m.report_end_date ? fmtDate(m.report_end_date) : '-'}
                        </span>
                        <br />
                        <small>{c.reportDays !== null ? `${c.reportDays} gün` : '-'}</small>
                      </td>
                      <td>{m.status || 'Aktif'}</td>
                      <td>
                        <div className="p360Actions">
                          <select value={m.status || 'Aktif'} onChange={(e) => updateMedicineStatus(m, e.target.value)}>
                            <option>Aktif</option>
                            <option>Pasif</option>
                            <option>Bitti</option>
                            <option>Doktor Stop</option>
                          </select>
                          <button onClick={() => deleteMedicine(m)}>Sil</button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <style jsx global>{`
        .p360Hero{display:flex;justify-content:space-between;gap:16px;background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:18px;margin-bottom:16px}
        .p360Hero a{color:#2563eb;text-decoration:none;font-weight:800}
        .p360Hero h2{margin:8px 0 0;color:#0f172a;font-size:30px;font-weight:900}
        .p360Hero p{margin:6px 0 0;color:#64748b}
        .p360PatientBox{background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:12px 16px;min-width:160px}
        .p360PatientBox span{display:block;color:#64748b;font-size:12px;font-weight:900}
        .p360PatientBox strong{display:block;color:#0f172a;font-size:18px;margin-top:4px}
        .p360Stats{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px;margin-bottom:16px}
        .p360Stat{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:16px}
        .p360Stat span{display:block;color:#64748b;font-size:13px;font-weight:800}
        .p360Stat strong{display:block;color:#0f172a;font-size:30px;margin-top:8px}
        .p360AlertPanel,.p360Panel{background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:16px;margin-bottom:16px}
        .p360AlertPanel h3,.p360Panel h3{margin:0 0 14px;color:#0f172a;font-size:20px}
        .p360AlertGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}
        .p360Alert{border-radius:16px;padding:14px;border:1px solid #e5e7eb}
        .p360Alert.ok{background:#f0fdf4;border-color:#bbf7d0}
        .p360Alert.warn{background:#fffbeb;border-color:#fde68a}
        .p360Alert.danger{background:#fef2f2;border-color:#fecaca}
        .p360Alert.neutral{background:#f8fafc}
        .p360Alert span{font-size:12px;font-weight:900;color:#64748b}
        .p360Alert strong{display:block;margin-top:5px;color:#0f172a}
        .p360Alert p{margin:5px 0 0;color:#475569}
        .p360Grid2{display:grid;grid-template-columns:2fr 1fr;gap:16px;margin-bottom:16px}
        .p360FormGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:12px}
        .p360Panel label{display:block;color:#475569;font-size:13px;font-weight:800;margin-bottom:6px}
        .p360Panel input,.p360Panel select{width:100%;height:42px;border:1px solid #cbd5e1;border-radius:12px;padding:0 10px;background:white;color:#0f172a}
        .p360Primary{height:44px;border:none;border-radius:12px;background:#2563eb;color:white;font-weight:900;padding:0 18px;cursor:pointer}
        .p360Info p{margin:0 0 10px;color:#334155}
        .p360PanelHead{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}
        .p360PanelHead h3{margin:0}
        .p360PanelHead span{color:#64748b;font-size:13px;font-weight:700}
        .p360TableWrap{overflow:auto}
        .p360Table{width:100%;border-collapse:collapse}
        .p360Table th,.p360Table td{border-bottom:1px solid #e5e7eb;text-align:left;padding:10px;vertical-align:top;font-size:14px}
        .p360Table th{background:#f8fafc;color:#475569;font-size:12px;text-transform:uppercase}
        .p360Badge{display:inline-block;padding:4px 8px;border-radius:999px;font-weight:900;font-size:12px;background:#f1f5f9;color:#334155}
        .p360Badge.ok{background:#dcfce7;color:#166534}
        .p360Badge.warn{background:#fef3c7;color:#92400e}
        .p360Badge.danger{background:#fee2e2;color:#991b1b}
        .p360Actions{display:flex;gap:8px}
        .p360Actions select{height:34px}
        .p360Actions button{height:34px;border:none;border-radius:10px;background:#fee2e2;color:#991b1b;font-weight:800;padding:0 10px;cursor:pointer}
        .p360Grid4{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
        .p360History{background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:16px}
        .p360History h3{margin:0 0 12px;color:#0f172a}
        .p360HistoryRow{border-top:1px solid #f1f5f9;padding:10px 0}
        .p360HistoryRow:first-of-type{border-top:none}
        .p360HistoryRow b{display:block;color:#0f172a}
        .p360HistoryRow span{display:block;color:#64748b;font-size:13px;margin-top:3px}
        .p360Empty{color:#64748b;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:10px}
        @media(max-width:1200px){.p360Stats{grid-template-columns:repeat(3,minmax(0,1fr))}.p360Grid2,.p360Grid4{grid-template-columns:1fr}.p360AlertGrid,.p360FormGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
        @media(max-width:700px){.p360Stats,.p360AlertGrid,.p360FormGrid{grid-template-columns:1fr}.p360Hero{flex-direction:column}}
      `}</style>
    </>
  );
}
...
function AlertCard({ type, title, desc, tone }: { type: string; title: string; desc: string; tone: string }) {
  return (
    <div className={`p360Alert ${tone}`}>
      <span>{type} Uyarısı</span>
      <strong>{title}</strong>
      <p>{desc}</p>
    </div>
  );
