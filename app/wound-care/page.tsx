...
export default function Page() {
  ...
  return (
    <Shell>
      <div className="hero">
        <h2>Bası / Pansuman Takibi</h2>
        <p>Bası yarası, yara yeri, evre, pansuman ve iyileşme takibi.</p>
      </div>
      ...
      <div className="panel">
        ...
        <div className="grid grid4">
          ...
          <div>
            <label>Yara Yeri</label>
            <input type="input" value={form.wound_location} onChange={(e) => setForm({ ...form, wound_location: e.target.value })} />
          </div>
          <div>
            <label>Evre</label>
            <input type="input" value={form.wound_stage} onChange={(e) => setForm({ ...form, wound_stage: e.target.value })} />
          </div>
          <div>
            <label>Pansuman</label>
            <input type="input" value={form.dressing_type} onChange={(e) => setForm({ ...form, dressing_type: e.target.value })} />
          </div>
          <div>
            <label>Not</label>
            <textarea value={form.wound_note} onChange={(e) => setForm({ ...form, wound_note: e.target.value })} />
          </div>
          <div>
            <label>Durum</label>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{formatValue(row.wound_location)}</td>
                  <td>{formatValue(row.wound_stage)}</td>
                  <td>{formatValue(row.dressing_type)}</td>
