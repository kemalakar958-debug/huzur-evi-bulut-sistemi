...
export default function Page() {
  ...
  return (
    <Shell>
      <div className="hero">
        <h2>Hastane Sevk Modülü</h2>
        <p>Sevk nedeni, hastane, refakatçi, gidiş-dönüş ve sonuç kaydı.</p>
      </div>
      ...
      <div className="panel">
        ...
        <div className="grid grid4">
          ...
          <div>
            <label>Gidiş Zamanı</label>
            <input type="datetime-local" value={form.transfer_at} onChange={(e) => setForm({ ...form, transfer_at: e.target.value })} />
          </div>
          <div>
            <label>Hastane</label>
            <input type="input" value={form.hospital_name} onChange={(e) => setForm({ ...form, hospital_name: e.target.value })} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="input" value={form.companion} onChange={(e) => setForm({ ...form, companion: e.target.value })} />
          </div>
          <div>
            <label>Ulaşım</label>
            <input type="input" value={form.transport_type} onChange={(e) => setForm({ ...form, transport_type: e.target.value })} />
          </div>
          <div>
            <label>Durum</label>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{formatValue(row.transfer_at)}</td>
                  <td>{formatValue(row.hospital_name)}</td>
                  <td>{formatValue(row.reason)}</td>
                  <td>{formatValue(row.companion)}</td>
