...
export default function Page() {
  ...
  return (
    <Shell>
      <div className="hero">
        <h2>Randevu / Doktor Takibi</h2>
        <p>Doktor randevusu, tetkik, kontrol ve sonucu kayıt altına al.</p>
      </div>
      ...
      <div className="panel">
        ...
        <div className="grid grid4">
          ...
          <div>
            <label>Tür</label>
            <input type="input" value={form.appointment_type} onChange={(e) => setForm({ ...form, appointment_type: e.target.value })} />
          </div>
          <div>
            <label>Bölüm</label>
            <input type="input" value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })} />
          </div>
          <div>
            <label>Doktor</label>
            <input type="input" value={form.doctor_name} onChange={(e) => setForm({ ...form, doctor_name: e.target.value })} />
          </div>
          <div>
            <label>Sonuç / Not</label>
            <textarea value={form.result_note} onChange={(e) => setForm({ ...form, result_note: e.target.value })} />
          </div>
          <div>
            <label>Durum</label>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{formatValue(row.appointment_at)}</td>
                  <td>{formatValue(row.appointment_type)}</td>
                  <td>{formatValue(row.department)}</td>
                  <td>{formatValue(row.doctor_name)}</td>
