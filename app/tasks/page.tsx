...
export default function Page() {
  ...
  return (
    <Shell>
      <div className="hero">
        <h2>Görev Takip</h2>
        <p>Pansuman, vital, kan şekeri, randevu ve bakım görevleri.</p>
      </div>
      ...
      <div className="panel">
        ...
        <div className="grid grid4">
          ...
          <div>
            ...
            <input type="input" value={form.task_type} onChange={(e) => setForm({ ...form, task_type: e.target.value })} />
          </div>
          <div>
            <label>Başlık</label>
            <input type="input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
          </div>
          <div>
            <label>Termin</label>
          ...
          <div>
            <label>Öncelik</label>
            <input type="input" value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} />
          </div>
          <div>
            <label>Atanan</label>
            <input type="input" value={form.assigned_to} onChange={(e) => setForm({ ...form, assigned_to: e.target.value })} />
          </div>
          <div>
            <label>Durum</label>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{formatValue(row.task_type)}</td>
                  <td>{formatValue(row.title)}</td>
                  <td>{formatValue(row.due_at)}</td>
                  <td>{formatValue(row.priority)}</td>
