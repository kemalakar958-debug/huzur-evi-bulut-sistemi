...
export default function Page() {
  ...
  const [form,setForm]=useState<any>({patient_id:'',task_type:'',title:'',due_at:'',priority:'',assigned_to:'',status:''});
  ...
  async function load(){
    ...
    const {data:r}=await supabase.from('care_tasks').select('*').order('created_at',{ascending:false}).limit(200);
    setRows(r||[]);
  }
  async function save(){
  ...
  async function complete(id:string){
    await supabase.from('care_tasks').update({status:'Tamamlandı', completed_at:new Date().toISOString()}).eq('id',id);
    await load();
  }
  return <Shell>
    <div className="hero"><h2>Görev Takip</h2><p>Pansuman, vital, kan şekeri, randevu ve bakım görevleri.</p></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Yeni Kayıt</h2><p>Hasta seçerek kayıt oluştur.</p></div></div>
      <div className="grid grid4"><div><label>Hasta</label><select value={form.patient_id} onChange={e=>setForm({...form,patient_id:e.target.value})}>{patients.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></div><div><label>Görev Türü</label><select value={form.task_type} onChange={e=>setForm({...form,task_type:e.target.value})}><option>Vital Kontrol</option><option>Kan Şekeri</option><option>Pansuman</option><option>Doktor Randevusu</option><option>Bakım</option><option>Evrak</option><option>Diğer</option></select></div><div><label>Başlık</label><input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} /></div><div><label>Termin</label><input type="datetime-local" value={form.due_at} onChange={e=>setForm({...form,due_at:e.target.value})} /></div><div><label>Öncelik</label><select value={form.priority} onChange={e=>setForm({...form,priority:e.target.value})}><option>Normal</option><option>Acil</option><option>Kritik</option></select></div><div><label>Atanan</label><input value={form.assigned_to} onChange={e=>setForm({...form,assigned_to:e.target.value})} /></div><div><label>Durum</label><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Bekliyor</option><option>Tamamlandı</option><option>İptal</option></select></div></div>
    ...
    <div className="panel"><div className="panelHead"><div><h2>Kayıtlar</h2><p>Son 200 kayıt</p></div></div>
      <div className="tableWrap"><table><thead><tr><th>Zaman</th><th>Hasta</th><th>Görev Türü</th><th>Başlık</th><th>Termin</th><th>Öncelik</th><th>Atanan</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString('tr-TR')} </td><td><b>{r.patient_name}</b></td><td>{r.task_type||'-'}</td><td>{r.title||'-'}</td><td>{r.due_at||'-'}</td><td>{r.priority||'-'}</td><td>{r.assigned_to||'-'}</td><td><span className={`pill ${r.status==='Tamamlandı'||r.status==='Aktif'||r.status==='Normal'?'ok':'warn'}`}>{r.status||'-'}</span></td></td><td>{r.status!=='Tamamlandı'?<button className="green" onClick={()=>complete(r.id)}>Tamamla</button>:'-'}</td></tr>)}{rows.length===0&&<tr><td colSpan={9}>Kayıt yok.</td></tr>}</tbody></table></div>
    </div>
  </Shell>
}
